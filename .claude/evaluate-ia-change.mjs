...
// product/ is the canonical product harness. Zones are derived from the actual
// product/ folder structure (folders are the source of truth). Adding a new
// product zone requires adding it here and updating README.md / the IA plan.
const allowedProductZones = new Set([
  "backlog",
  "branding",
  "content",
  "data",
...
for (const entry of entries) {
  ...
  // Non-live paths (any `_`-prefixed folder at the root or deeper) are logged
  // and skipped before zone validation: e.g. `_archive/`, `_inbox/`,
  // `product/_recovered/`, `product/branding/_superseded/`, `docs/_archive/`.
  if (rootEntry.startsWith("_") || isNonLivePath(normalized)) {
  ...
  if (rootEntry === "product") {
    // The product harness is enforced STRUCTURALLY only: every nested file must
    // live under a canonical product zone (top-level product files like
    // backlog.md are allowed). The migrated corpus keeps its legacy filenames
    // and frontmatter, so the docs/-scoped content-style rules below do not
    // apply here — folder placement is the contract.
    if (segments.length > 2 && !allowedProductZones.has(segments[1])) {
      fail(normalized, `product zone '${segments[1]}' is not canonical; update IA and README.md first.`);
    }
    continue;
  }
