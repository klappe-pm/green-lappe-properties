...
Green Property Management should add a childcare-operator property line: single-family homes leased to a licensed in-home childcare provider who lives in the home and runs a licensed Family Child Care Home (FCCH) there, with free or subsidized housing forming part of the operator's compensation in exchange for a contracted child-enrollment (placement) commitment. This turns a property from a thin residential-rent asset into a childcare-business host asset, and it aligns GPM's management-and-placement competence with a demand-constrained market the market study values at roughly $0.8B served and about $1.2B in total demand.
...
| Initiative | Before (current GPM) | After (childcare-operator model) | Targeted outcome | How to measure | Plan document |
|---|---|---|---|---|---|
| Property use | Home leased to a family as a residential rental | Home leased to a licensed FCCH operator who lives in and operates it | Higher income per property and a defensible niche | Net operating income per property vs a comparable residential lease; count of childcare-host properties live | [[childcare/property-sourcing-and-underwriting\|property sourcing and underwriting]] |
...
This strategy changes GPM's public and internal positioning, so the following source-of-truth artifacts must be reconciled in the same cycle, not after:

- `src/config/site.ts` and the owners pricing page describe GPM only as residential property management with a 9%-plus-placement model. If the childcare line ships, these need a second, clearly separated service description, and the pricing model must state the enrollment-linked component. Do not overwrite the residential model; add the childcare model beside it.
...
- D3 revenue split: the enrollment-linked component's size and whether GPM takes equity in the childcare business or only a fee.
- D4 capital model: does GPM own the homes, master-lease them, or manage owner-supplied homes into the childcare use.
